...
**Источник правды — одна PostgreSQL.** Каталог, корзина, заказы, пользователи Telegram, FAQ и настройки бота лежат в общей схеме. Так WebApp и бот не расходятся по состоянию: не нужен отдельный сервис синхронизации и нет риска «двух версий» заказа или корзины.

**Граница WebApp → бэкенд** — только **REST** (`/api/webapp/…`). У мини-приложения нет доступа к БД; пользователь привязывается к записям через проверку подписи **Telegram initData** на Django. Слой API задаёт контракт и права для клиента в браузере/WebView.

**Граница бот → данные** — **прямой асинхронный доступ** к тем же таблицам (SQLAlchemy). Сценарии в чате (каталог, корзина, FSM, рассылки) не обязаны проходить через HTTP к Django: бот — отдельный процесс с тем же доменом данных, что и админка.

**Граница Django → бот** — **исходящий HTTP** на внутренний адрес бота (`BOT_INTERNAL_URL`, в т.ч. `/notify`). Событие возникает при обработке запроса к Django (например, новый заказ из API или смена статуса в админке), а доставить сообщение в Telegram должен уже запущенный процесс бота. Для ожидаемой нагрузки магазина это проще отдельной брокерской очереди; при росте трафика тот же контракт можно вынести за продюсер/консьюмер.

**Точка входа снаружи** — **nginx**: один origin для Mini App (HTML/статика Next.js), прокси на Django (API, админка, медиа) и согласованные заголовки для HTTPS за прокси.
